...
mailRouter.get("/", async (req,res) => {
  ...
  const [mail,total] = await Promise.all([
    prisma.mail.findMany({where,skip,take:parseInt(limit),orderBy:{createdAt:"desc"},select:{id:true,subject:true,senderName:true,senderHandle:true,senderVerified:true,bodyPreview:true,mailType:true,isRead:true,isStarred:true,aiSummary:true,aiUrgency:true,deliveredAt:true}}),
...
mailRouter.get("/stats", async (req,res) => {
  ...
  const [total,unread] = await Promise.all([
    prisma.mail.count({where:{userId,isTrashed:false}}),
    prisma.mail.count({where:{userId,isRead:false,isTrashed:false}})
  ])
...
mailRouter.get("/:id", async (req,res) => {
  const userId = uid(req)
  if(!userId) return res.status(401).json({error:"Unauthorized"})
  const mail = await prisma.mail.findFirst({where:{id:req.params.id,userId},select:{id:true,subject:true,body:true,senderName:true,senderHandle:true,senderVerified:true,mailType:true,isRead:true,isStarred:true,aiSummary:true,aiUrgency:true,deliveredAt:true,deliveryToken:true,payload:true}})
...
mailRouter.patch("/batch", async (req,res) => {
  ...
  await prisma.mail.updateMany({where:{id:{in:ids},userId},data:map[action]})
